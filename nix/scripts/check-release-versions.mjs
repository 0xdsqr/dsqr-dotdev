...
for (const [app, { chart: chartPath, deployment }] of releaseByApp) {
  ...
  const imageVersion = productionValues.match(/^\s*version:\s*["']?([^\s"']+)/m)?.[1]
  const imageDigest = productionValues.match(
    /^\s*digest:\s*["']?(sha256:[0-9a-f]{64})/m,
  )?.[1]
  if (imageVersion !== packageVersion) {
    errors.push(
      `${productionValuesPath} image.version is ${imageVersion}; expected ${packageVersion}`,
    )
  }
  if (!imageDigest) {
    errors.push(`${productionValuesPath} must pin a lowercase sha256 image digest`)
  }
}
