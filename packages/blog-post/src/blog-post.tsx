...
interface BlogPostCategoryProps
  extends React.HTMLAttributes<HTMLSpanElement>, VariantProps<typeof blogPostCategoryVariants> {}
...
interface BlogPostCalloutProps
  extends React.HTMLAttributes<HTMLDivElement>, VariantProps<typeof blogPostCalloutVariants> {
  label?: string
}
...
const BlogPostCodeBlock = React.forwardRef<HTMLDivElement, BlogPostCodeBlockProps>(
  (
    { className, code, language: _language, showLineNumbers = true, highlightLines = [], ...props },
    ref,
  ) => {
        ...
        <figure ref={ref} className={cn("my-8 group", className)}>
          ...
          {(caption || credit) && (
            <figcaption className="mt-3 space-y-1">
              {caption && (
                <p className="text-sm text-muted-foreground font-mono text-center">{caption}</p>
              )}
              {credit && (
                <p className="text-xs text-muted-foreground/60 font-mono text-center">{credit}</p>
              )}
            </figcaption>
          )}
        </figure>
