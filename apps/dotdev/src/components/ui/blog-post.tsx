...
function useBlogPost() {
  ...
  if (!context) {
    throw new Error("Blog post components must be used within a BlogPost provider")
  }
  return context
}
...
const BlogPost = React.forwardRef<HTMLElement, BlogPostProps>(
  ({ className, children, initialLikes = 0, ...props }, ref) => {
    ...
    return (
      <BlogPostContext.Provider value={{ liked, setLiked, bookmarked, setBookmarked, likeCount, setLikeCount }}>
        <article ref={ref} className={cn("w-full max-w-none", className)} {...props}>
          {children}
        </article>
...
const BlogPostHeader = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => <header ref={ref} className={cn("mb-10", className)} {...props} />,
)
...
interface BlogPostBackLinkProps extends React.AnchorHTMLAttributes<HTMLAnchorElement> {
  asChild?: boolean
}

const BlogPostBackLink = React.forwardRef<HTMLAnchorElement, BlogPostBackLinkProps>(
...
const BlogPostCover = React.forwardRef<HTMLDivElement, BlogPostCoverProps>(
  ({ className, src, alt, aspectRatio = "wide", ...props }, ref) => {
    ...
    return (
      <div
        ref={ref}
        className={cn("relative w-full overflow-hidden rounded-xl border border-border mb-8 group", className)}
      >
...
const BlogPostMeta = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div ref={ref} className={cn("flex flex-wrap items-center gap-3 mb-4", className)} {...props} />
  ),
)
...
const BlogPostCategory = React.forwardRef<HTMLSpanElement, BlogPostCategoryProps>(
  ({ className, variant, ...props }, ref) => (
    <span ref={ref} className={cn(blogPostCategoryVariants({ variant }), className)} {...props} />
  ),
)
...
const BlogPostDate = React.forwardRef<HTMLSpanElement, BlogPostDateProps>(
  ({ className, date, showIcon = false, ...props }, ref) => {
    ...
    return (
      <span
        ref={ref}
        className={cn("text-sm text-muted-foreground font-mono flex items-center gap-1.5", className)}
        {...props}
      >
...
interface BlogPostReadingTimeProps extends React.HTMLAttributes<HTMLSpanElement> {
  minutes: number
  showIcon?: boolean
}

const BlogPostReadingTime = React.forwardRef<HTMLSpanElement, BlogPostReadingTimeProps>(
...
const BlogPostTitle = React.forwardRef<HTMLHeadingElement, React.HTMLAttributes<HTMLHeadingElement>>(
  ({ className, ...props }, ref) => (
    <h1
      ref={ref}
      className={cn(
  ...
  ({ className, name, handle, avatarUrl, ...props }, ref) => (
    <div ref={ref} className={cn("flex items-center gap-3", className)} {...props}>
      ...
      <div>
        ...
        {handle && <p className="text-xs text-muted-foreground font-mono">{handle}</p>}
      </div>
    </div>
  ),
)
...
const BlogPostEngagement = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn("flex flex-wrap items-center justify-between gap-4 py-4 border-y border-border", className)}
      {...props}
    />
  ),
)
...
const BlogPostActions = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
...
const BlogPostLikeButton = React.forwardRef<HTMLButtonElement, React.ButtonHTMLAttributes<HTMLButtonElement>>(
  ({ className, ...props }, ref) => {
...
interface BlogPostCommentButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {
  count?: number
}

const BlogPostCommentButton = React.forwardRef<HTMLButtonElement, BlogPostCommentButtonProps>(
...
const BlogPostBookmarkButton = React.forwardRef<HTMLButtonElement, React.ButtonHTMLAttributes<HTMLButtonElement>>(
  ({ className, ...props }, ref) => {
...
const BlogPostShareButton = React.forwardRef<HTMLButtonElement, React.ButtonHTMLAttributes<HTMLButtonElement>>(
  ({ className, ...props }, ref) => (
    <button
      ref={ref}
      type="button"
...
const BlogPostTags = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => <div ref={ref} className={cn("flex flex-wrap gap-2 mt-4", className)} {...props} />,
)
...
const BlogPostContent = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
...
const blogPostCalloutVariants = cva("my-6 p-4 rounded-r-lg font-mono text-sm border-l-4", {
  variants: {
    variant: {
      ...
      warning: "border-amber-500 dark:border-amber-400 bg-amber-50/50 dark:bg-amber-950/20",
      important: "border-red-500 dark:border-red-400 bg-red-50/50 dark:bg-red-950/20",
      purple: "border-purple-500 dark:border-purple-400 bg-purple-50/50 dark:bg-purple-950/20",
    },
  },
  defaultVariants: {
    variant: "note",
...
const BlogPostCallout = React.forwardRef<HTMLDivElement, BlogPostCalloutProps>(
  ({ className, variant = "note", label, children, ...props }, ref) => {
    const defaultLabel = variant ? variant.charAt(0).toUpperCase() + variant.slice(1) : "Note"

    return (
      <div ref={ref} className={cn(blogPostCalloutVariants({ variant }), className)} {...props}>
        <p className={cn("text-xs font-semibold uppercase tracking-wider mb-2", calloutLabelStyles[variant || "note"])}>
          {label || defaultLabel}
        </p>
...
const BlogPostCodeBlock = React.forwardRef<HTMLDivElement, BlogPostCodeBlockProps>(
  ({ className, code, language, showLineNumbers = true, highlightLines = [], ...props }, ref) => {
            ...
            <code>
              {lines.map((line, index) => {
                ...
                return (
                  <div
                    key={index}
                    className={cn("px-4 flex", isHighlighted && "bg-purple-500/10 border-l-2 border-purple-500")}
                  >
                    {showLineNumbers && (
                      <span
                        className={cn(
                          "w-8 flex-shrink-0 select-none text-right pr-4 tabular-nums",
                          isHighlighted ? "text-purple-500" : "text-muted-foreground/40",
                        )}
                      >
                        {lineNumber}
                      </span>
...
const BlogPostImage = React.forwardRef<HTMLElement, BlogPostImageProps>(
  (
    { className, src, alt, caption, credit, aspectRatio = "auto", zoomable = true, animated = false, ...props },
    ref,
  ) => {
        ...
        <figure ref={ref} className={cn("my-8 group", className)}>
          ...
          {(caption || credit) && (
            <figcaption className="mt-3 space-y-1">
              {caption && <p className="text-sm text-muted-foreground font-mono text-center">{caption}</p>}
              {credit && <p className="text-xs text-muted-foreground/60 font-mono text-center">{credit}</p>}
            </figcaption>
          )}
        </figure>
...
const BlogPostSeparator = React.forwardRef<HTMLHRElement, React.HTMLAttributes<HTMLHRElement>>(
  ({ className, ...props }, ref) => (
    <hr ref={ref} className={cn("my-12 border-t border-border", className)} {...props} />
  ),
)
